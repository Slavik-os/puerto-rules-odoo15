...
class FolderManagment(models.Model):
    ...
    #Select rules
    rules = fields.Many2one("rules.creation",
                             tracking=True,string="Selection une regles")
    ...
    def _apply_rules(self):
        for rec in self:
            # Bypass defaults
            if rec.rules.rules == False:
                return
            rules_l = rec.rules.rules.split("then")
            print("Rule hit !!!!")
            print(rules_l)
            #Split to parse ['condition','action','case']
            #Condtion to apply
            action = rules_l[1]
            try:
                _exec = action.split("else")
                print("TO EXECUTE : {} !!!!".format(_exec))
                condition = rules_l[0]
                print(condition.strip("if"))
                if eval(condition.strip("if")):
    ...
    n_dum = fields.Char(string="Numero Dum")
    l_dum = fields.Many2one('dum.number', string="N °Dum",tracking=True)
    ...
    #destinataire = fields.Selection(related="_expiditeur.expiditeur_ids",string="Déstinataire",readonly=False,tracking=True)
    destinataire  = fields.Char(String="Déstinataire")

    # Serie number
    n_serie = fields.Char(string="N°Serie",tracking=True,readonly=True)

    # Bureaux dedouan
    _bureax_dedouane = fields.Many2one('bureaux.dedouan',tracking=True)
    bureax_dedouane = fields.Selection(related='_bureax_dedouane.bureaux_dedouan',string='Bureau de dédouanement',readonly=False,tracking=True)
    #bureax_dedouane = fields.Char(string="Bureau de dédouanement")
    # Arrondissement
    ...
    #                                         copy=True,readonly=False,tracking=True)
    bureaux_destinataire = fields.Char(String="Bureau de Déstination")
    ...
    expiditeur = fields.Selection(related="_expiditeur.expiditeur_ids",
                                   readonly=False,copy=True ,tracking=True)
    ...
    upload_files_ids = fields.Many2many('pieceattach.control',string="Attacher une piece",tracking=True)


    # Les Document Annexees :
    attach_documents = fields.Many2many("attach.document.managment",tracking=True,readonly=False)

    # List des Factures
    list_factures = fields.Many2many("invoice.list.controll",tracking=True,readonly=False,store=True)
